...
let newList = list // first number
let reversedNewList = list.split('').reverse().join('') // last number

function regexList(wordIntoNumber, list) {
  for (let _ in wordIntoNumber) {
    const regex = new RegExp(Object.keys(wordIntoNumber).join('|'), 'g')
    return list.replace(regex, (key) => wordIntoNumber[key])
  }
}

const splitList = regexList(wordToNumber, newList).split(/\r?\n/)

const reversedSplitList = regexList(reversedWordToNumber, reversedNewList)
  .split('')
  .reverse()
  .join('')
  .split(/\r?\n/)

const numbers = splitList.map((e) => {
  const numberList = e.match(/\d+/g)
  return numberList === null ? 0 : numberList.join('')
})

const lastNumbers = reversedSplitList.map((e) => {
  const lastNumberList = e.match(/\d+/g)
  return lastNumberList === null ? 0 : lastNumberList.join('')
})
...
for (let i in numbers) {
  sum +=
    numbers[i].length > 1
      ? Number(numbers[i][0] + lastNumbers[i][lastNumbers[i].length - 1])
