const input = `Game 1: 4 blue; 1 green, 2 red; 4 blue, 1 green, 6 red
...
for (i in games) {
  ...
  finalResult.push(result[i][1].split(';'))
}
let power = 0
for (let i = 0; i < finalResult.length; i++) {
  ...
  let highestRed = 0,
    highestGreen = 0,
    highestBlue = 0
  finalResult[i].forEach((item, index, array) => {
    const red = parseInt(item.match(/(\d+) red/g)?.[0]) || 0
    const green = parseInt(item.match(/(\d+) green/g)?.[0]) || 0
    const blue = parseInt(item.match(/(\d+) blue/g)?.[0]) || 0
    ...
    if (index === array.length - 1) {
      console.log(`R: ${highestRed}, B: ${highestBlue}, G: ${highestGreen}`)
      power = power + highestRed * highestBlue * highestGreen
      if (isTrue) {
        sum += i + 1
      }
    }
